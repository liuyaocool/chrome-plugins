...
function addM3u8(performance) {
    let url = performance.name, id = url;
    if (M3U8_MAP[id] || !url.startsWith('http')) return;
    let type = new URL(url).pathname.split('.'), name;
    try { name = window.top.document.title; } catch (e) { console.warn(e); }
    name = name || 'video';
    // console.log(type[type.length-1]);
    switch (type = type[type.length-1]) {
        case 'm3u8': M3U8_MAP[id] = new M3u8Handler(url, name); break;
        case 'mp4': M3U8_MAP[id] = new Mp4Handler(url, name); break;
        case 'mp4/': M3U8_MAP[id] = new Mp4Handler(url, name); break;
        default: return;
    }
}
    ...
    init() {
        ...
        ajax({
            ...
            success: m3u8Str => {
                ...
                // 提取加密信息
                if (m3u8Str.indexOf('#EXT-X-KEY') >= 0) {
                    that.aesMethod = (m3u8Str.match(/(.*METHOD=([^,\s]+))/) || ['', '', ''])[2];
                    let aesUrl = (m3u8Str.match(/(.*URI="([^"]+))"/) || ['', '', ''])[2];
                    aesUrl = this.genPartUrl(aesUrl, m3u8URL);
                    let ivStr = (m3u8Str.match(/(.*IV=([^,\s]+))/) || ['', '', ''])[2];
                //     that.aesIv = ivStr ? new TextEncoder().encode(ivStr).buffer : null;
                //     that.aesKey = ajax('file', aesUrl);
                //     that.decryptor = new AESDecrypt();
                //     that.decryptor.expandKey(that.aesKey.buffer);
                }
                // 提取 ts 视频片段地址
                m3u8Str.split('\n').forEach(line => {
                    if (!(line = line.trim())) return;
                    if ('#' !== line[0]) {
                        tsUrls.push(tdHtm = that.genPartUrl(line, m3u8URL));
